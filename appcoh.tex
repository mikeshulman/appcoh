...
Let$\U_{\set{R}}$ be the universe of types homeomorphic (\Chandrika{as opposed to homotopy equivalent}) to $\set{R}$. The tautological bundle $\text{taut}^1: \set{RP}^1 \rightarrow \U_{\set{R}}$ is defined by the constructors $\set{I} \mapsto \set{R}$ and $(\{0\} \times \set{I}) \mapsto \set{R}$, and the generator of $\pi_1(\set{RP}^1) \mapsto -Id$
