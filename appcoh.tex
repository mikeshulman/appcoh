...
Based on work of Shulman \cite{}, we have $\shape(\set{S}^{n-1} \times \set{I}) =S^{n-1}$ and $\shape (\set{D}^{n})$ is the unit type. Next, we find $\shape (\text{taut}^{n-1})$ so that we may understand $ \shape \set{RP}^n$.
