...
The tautological bundle $\mathcal{M}^n$ is defined by the map $m^n: \set{RP}^n \rightarrow \U_{\set{R}}$ which is given by the constructors of $m^{n-1}$ (which defines $\mathcal{M}^{n-1}$), as well as $\set{D}^n \mapsto \set{R}$, together with the equivalence $\set{R}$, $\lambda x.x$ for every point of $\set{S}^{n-1} \times \set{I}$.
\end{definition}
\Chandrika{This should not be the trivial bundle because $\mathcal{M}^{n-1}$ is not trivial, and we use its constructors.}
...
Because pushouts are stable under pullback (due to the adhesiveness of this category \Chandrika{add other words from email from Mike to Dan}), it suffices to show that the vertical faces of the cube are pullbacks in order to prove that the top face of the cube is a pushout. The vertical faces are pullbacks because composition with the classifying map $m^n$ yields a pullback of the total spaces.
...
\Amelia{(3/15/22) We need to say how we're viewing $\ZZtwo$ as a space below since we later refer to this as a functor landing in spaces}
...
\]
\Amelia{Do we know that $K(-,n)$ is a functor in HoTT? It is a functor, but I'm not sure we ever proved it and I don't think it's explicitly stated in Finster-Licata.}
