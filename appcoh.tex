...
% TODO input any sharp, flat terms in as needed.
...
arguement as in the classical case.

Next, we show that $\neg R \Rightarrow O$.  

%% TODO show that -R => O, will need sharp and flat to get LEM?
%% Want classically provable but not intuitionally provable:
%% notP -> Q isEquivTo notQ -> P. Note that, we might have LEM flatly/sharply
...
\sum_{x \in \mathcal{M}^{n-1}}  f \circ s (x) \arrow[rr, crossing over, near start, "\lambda"] \arrow[dd, "p_3"]	&																					& \sum_{x \in \set{RP}^n}f(x)				&	\\
...
g \circ h((c, r), b, k)	& = g(b, r) \\
				...
				& = ((c,r), b, \text{ref;}_{s(b)}),
\end{align*}
since $s(b) = c$ (as witnessed by $k$). It remains to show that $\text{refl}_{s(b)}=k$, but this must be true because $\set{RP}^n$ is a set. 
