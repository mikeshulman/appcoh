...
A \emph{cohesive topos} is a pair of topoi
$ ( \cat{E}, \cat{S} ) $ equipped with a string of
adjunctions 
\[
...
the objects of $ \cat{S} $ should be thought of as
...
\end{quote}
On the other hand, the objects of $ \cat{E} $ should be
thought of as abstract sets together with a sort of
\emph{cohesion} between the ``dots''.  Today, one might call
this cohesion a topology. Also, we might consider a set
defined recursively to have a cohension arising from the
relationship established between points as they become
defined into existence.  The definition above axiomatizes
the various forms that cohesion may take through the adjoint
string. Lawvere originally gave the functors comprising that
adjoint string the more evokative names
\begin{center}
  components $ \dashv $ discrete $ \dashv $ points $ \dashv
  $ codiscrete.
\end{center}
The components functor tells us which points are ``stuck''
together through the cohesion. The discrete functor endows a
set with the minimal amount of cohesion. The points functor
removes any cohesion and returns the mere ``bag of dots''
underying a cohesive space. The codiscrete functor endows a
set with the maximum cohesive structure.  
